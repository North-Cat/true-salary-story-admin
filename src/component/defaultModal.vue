<template>
  <div v-if="dialogStore.isOpen" class="fixed inset-0 flex items-center justify-center bg-black-3 bg-opacity-50">
...
function clickComfirm() {
  if (dialogStore.dialogInfo.confirmFunc) {
    dialogStore.dialogInfo.confirmFunc();
  }
  dialogStore.close();
}
function clickCancel() {
  if (dialogStore.dialogInfo.cancelFunc) {
